import { parseURN, ParsingError } from '../src/urn-parser';
...
describe('Test urn-parser functionality', () => {
  ...
  test('should not parse malformed urn', () => {
    expect(() => parseURN('betriebsmittel:read')).toThrow(ParsingError);
    expect(() => parseURN('ams::read')).toThrow(ParsingError);
    expect(() => parseURN('ams::')).toThrow(ParsingError);
    expect(() => parseURN('ams::{read,write}')).toThrow(ParsingError);
  });
